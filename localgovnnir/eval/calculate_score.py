...
def format_result(result, correct_As, target_qs, target_as, correct_ABCs, top1_check, english):
    str_result = ""
    if not top1_check:
        str_result += "---------------------------\n"
    if not english:
    ...
    for i_result in result["result"]:
        if i_result["rank"] > (10000 if not top1_check else 1):
            break
        if top1_check:
            str_result += "{}\tQ:{}\tA:{}\n".format(
                    ('#' if i_result["q_id"] in correct_As else '') +\
                            str(i_result["rank"]),
                            str(i_result["question"]).replace(" ", ""),
                    str(target_as[int(i_result["q_id"])]).replace(" ", "")[:120]
                    )
        else:
            str_result += "{}\t{}\t{}\n".format(
                    ('#' if i_result["q_id"] in correct_As else '') +\
...
def main(args):
        ...
        if line[:4] == "----":
            if len(result) != 0:
                if args.english:
                    ...
                    t_question = result["question"].replace(" ", "")
                ...
                if rrrr:
                    r, str_result, Pn, AP, RR, DCG = rrrr
    ...
    if args.score:
        ...
        print("All : {}".format(stat["TP"] + stat["FP"]))
